...
loyaltyPlugin.prototype.getData = function( a, cb ){ 
  console.log(a, cb);
  mysqlUtils.query(this.connection, 'SELECT users.uid FROM users LEFT JOIN #__user_rewards ON #__user_rewards.user_id=users.uid WHERE #__user_rewards.id IS NULL AND FROM_UNIXTIME(users.created)<DATE_SUB(NOW(), INTERVAL 3 YEAR)', config.plugins.loyalty.prefix, function(err, rows) {
    cb(err, rows);
  });
}
