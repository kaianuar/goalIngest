...
loyaltyPlugin.prototype.getReward = function(key, cb) {
  ...
  connection.connect(function(err) {
    if(err) {
      ...
      mysqlUtils.query(connection, "SELECT id FROM #__rewards WHERE `reward_key`='"+key+"' LIMIT 0,1", config.plugins.loyalty_2.prefix, function(err, rows) {
        if(!err){
          if(rows.length) {
            cb(null, rows[0].id);
          } else {
...
loyaltyPlugin.prototype.getData = function( a, cb ){ 
  console.log(a, cb);
  mysqlUtils.query(this.connection, 'SELECT users.uid FROM users LEFT JOIN #__user_rewards ON #__user_rewards.user_id=users.uid WHERE #__user_rewards.id IS NULL AND FROM_UNIXTIME(users.created)<DATE_SUB(NOW(), INTERVAL 2 YEAR)', config.plugins.loyalty_2.prefix, function(err, rows) {
    cb(err, rows);
  });
}
...
loyaltyPlugin.prototype.reward = function(ids, cb) {
  if(ids.length) {
    ...
    mysqlUtils.query(this.connection, query, config.plugins.loyalty_2.prefix, cb);
  } else {
